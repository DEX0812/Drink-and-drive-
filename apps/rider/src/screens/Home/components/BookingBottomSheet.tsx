...
export default function BookingBottomSheet({ onBook }: Props) {
  ...
  const estimateData = estimate?.estimates?.[serviceLevel];
  const fare = estimateData?.total || 0;
  const breakdown = estimateData?.breakdown || { base: 0, distance: 0, time: 0, platform: 0 };
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
          ...
          {expanded && (
            <Animated.View style={styles.expandedContent}>
              ...
              {/* Advanced Estimate Card with Breakdown */}
              <View style={[styles.estimateCard, { backgroundColor: theme.surface, borderColor: theme.border }]}>
                   {loadingEstimate ? (
                     ...
                     <View style={styles.estimateInner}>
                        <View style={{ flex: 1 }}>
                           <Text style={[typography.label, { color: theme.textSecondary, fontSize: 9, letterSpacing: 1 }]}>TOTAL ESTIMATE</Text>
                           <View style={{ flexDirection: 'row', alignItems: 'baseline', marginTop: 4 }}>
                              <Text style={[typography.h1, { color: theme.text, fontSize: 32, fontWeight: '900' }]}>₹{fare}</Text>
                              <Text style={[typography.body, { color: theme.textSecondary, fontSize: 12, marginLeft: 8 }]}>incl. taxes</Text>
                           </View>
                           ...
                           <View style={styles.breakdownList}>
                              <View style={styles.breakdownItem}>
                                 ...
                                 <Text style={styles.breakdownText}>Base: ₹{breakdown.base}</Text>
                              </View>
                              <View style={styles.breakdownItem}>
                                 <View style={[styles.dot, { backgroundColor: '#34C759' }]} />
                                 <Text style={styles.breakdownText}>Dist: ₹{breakdown.distance}</Text>
                              </View>
                              <View style={styles.breakdownItem}>
                                 <View style={[styles.dot, { backgroundColor: '#007AFF' }]} />
                                 <Text style={styles.breakdownText}>Time: ₹{breakdown.time}</Text>
                              </View>
                           </View>
                        </View>
                        <View style={styles.metaBox}>
                           <View style={[styles.metaRow, { backgroundColor: theme.background, padding: 8, borderRadius: 12 }]}>
                               <Navigation size={12} color={theme.primary} />
                               <Text style={[typography.body, { color: theme.text, fontSize: 11, fontWeight: '700', marginLeft: 6 }]}>{estimate?.distanceKm || '--'} km</Text>
                           </View>
                           <View style={[styles.metaRow, { backgroundColor: theme.background, padding: 8, borderRadius: 12 }]}>
                               <Clock size={12} color={theme.primary} />
                               <Text style={[typography.body, { color: theme.text, fontSize: 11, fontWeight: '700', marginLeft: 6 }]}>{estimate?.durationMin || '--'} min</Text>
                           </View>
                        </View>
                     </View>
                   )}
...
const styles = StyleSheet.create({
  ...
  estimateInner: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' },
  breakdownList: { flexDirection: 'row', gap: 12, marginTop: 16 },
  breakdownItem: { flexDirection: 'row', alignItems: 'center' },
  dot: { width: 4, height: 4, borderRadius: 2, marginRight: 6 },
  breakdownText: { fontSize: 10, color: '#888', fontWeight: '600' },
  metaBox: { gap: 10, alignItems: 'flex-end' },
  metaRow: { flexDirection: 'row', alignItems: 'center', minWidth: 80 },
  mainBtn: { height: 64, borderRadius: 24, alignItems: 'center', justifyContent: 'center', shadowColor: '#000', shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.2, shadowRadius: 12, elevation: 8 },
