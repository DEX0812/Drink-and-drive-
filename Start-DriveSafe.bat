...
if exist "apps\rider\.expo" rmdir /s /q "apps\rider\.expo" >nul 2>&1
if exist "apps\driver\.expo" rmdir /s /q "apps\driver\.expo" >nul 2>&1
...
call npx prisma db push --schema packages/db/prisma/schema.prisma --accept-data-loss >nul 2>&1
call npx prisma generate --schema packages/db/prisma/schema.prisma >nul 2>&1
echo          Prisma Synchronized.
timeout /t 2 /nobreak >nul

:: Step 2.5: Synchronize Local Networking IP
echo [2.5/4] Aligning Dev Networking IP...
call node update-ip.js
timeout /t 2 /nobreak >nul
